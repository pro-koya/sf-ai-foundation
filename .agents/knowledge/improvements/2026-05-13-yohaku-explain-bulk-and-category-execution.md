...
type: improvement
date: 2026-05-13
title: /yohaku-explain に「一括」と「資材カテゴリ単位」の実行モードを追加
status: idea
tags: [v0.3.0, user-requested, v0.4.0-candidate, yohaku-explain, ux, bulk-execution]
---

# /yohaku-explain に「一括」と「資材カテゴリ単位」の実行モードを追加

## Before

現状の `/yohaku-explain <kind> <fqn>` は **1 つの資材 (fqn)** を必須引数として要求する。複数の Apex クラス・複数の Object をまとめて更新したい場合、利用者は資材ごとに個別に slash command を実行する必要があり、初回 onboarding 後の "全体一斉更新" や、特定種類だけの再生成 (例: `apexClass` 全部) で手数がかかる。
...
```
/yohaku-explain --kind apexClass --all
/yohaku-explain --kind customObject --all
/yohaku-explain --kind flow --all
```
...
- `yohaku explain-write` に `--all` / `--kind-only` フラグを追加 (CLI 層)
- `/yohaku-explain` skill (`.claude/commands/yohaku-explain.md`) の Step 1 引数解釈を拡張
...
**v0.3.0 Out of Scope**: 本機能は `/yohaku-explain` 機能追加 + 新サブコマンド相当 + 新フラグ群であり、`decisions/2026-05-10-v0.3.0-internal-validation-plan.md` §Out of Scope の **「AI 推論基盤の機能拡張」「新サブエージェント / 新 slash command の追加」「`/yohaku-explain` 機能追加」** に明確に抵触する。
...
1. 利用者が `kind apexClass` の全 fqn を `yohaku graph query` で取得
2. AI に「このリストを上から順に `/yohaku-explain apexClass <fqn>` で処理して」と依頼
