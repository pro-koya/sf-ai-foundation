...
type: improvement
date: 2026-05-09
title: Phase 15 検証で見えた `/yohaku-explain` の改善点 4 件
status: backlog
tags: [phase-15, yohaku-explain, improvements, ux]
---
...
**現状**: `yohaku explain-write --kind X --fqn Y --input Z.json` は即座に Markdown を更新する。
...
**改善案**: `--auto-kind` フラグまたはデフォルト挙動として、`--fqn` だけ与えれば `yohaku graph query` で kind を自動判定する経路を追加。例: `AccountBalanceService` → apex_classes に存在 → `apexClass` として処理。
