...
利用者環境 (`/Users/koya1104/.nodebrew/current/bin/sfai` 経由) で `sfai init --profile minimal --project-name sfai-trial --language ja` を実行すると、何も出力されず exit 0 で終了。`.claude/` `.agents/` 等の生成物が一切作られない。
...
- `process.argv[1]` = `/Users/koya1104/.nodebrew/current/bin/sfai` (symlink パスのまま)
- `import.meta.url` = `file:///Users/koya1104/Desktop/SF-AI-Foundation/packages/sfai-core/dist/cli.js` (resolve 済み)
...
- `which sfai` の出力が `/Users/koya1104/.nodebrew/current/bin/sfai` だった (symlink ヒント)
