...
type: pitfall
date: 2026-05-07
title: npm link 経由の yohaku が silent 終了するバグ (isDirectInvoke 比較が symlink を考慮していなかった)
severity: critical
tags: [phase-2, cycle-2-1, ci, cli, npm-link, symlink]
---

# npm link 経由の yohaku が silent 終了するバグ

## 何が起きたか

利用者環境 (`/Users/developer/.nodebrew/current/bin/yohaku` 経由) で `yohaku init --profile minimal --project-name yohaku-trial --language ja` を実行すると、何も出力されず exit 0 で終了。`.claude/` `.agents/` 等の生成物が一切作られない。

`yohaku version` ですら出力なし、というのが診断の決定打となった。

## 根本原因

`packages/core/src/cli.ts` の末尾で、ESM スクリプトが直接実行されたかを判定する従来の慣用コード:
...
- `process.argv[1]` = `/Users/developer/.nodebrew/current/bin/yohaku` (symlink パスのまま)
- `import.meta.url` = `file:///Users/developer/Desktop/yohakuforce/packages/core/dist/cli.js` (resolve 済み)

両者が文字列一致しないため `main()` が呼ばれず、cli.js が「ロードされただけで何もしない」状態になる。

## どう気づいたか

- 利用者から「`yohaku init` 実行後に `.claude/` 等が作られない」報告
- 私の手元では `node dist/cli.js init ...` で正常動作 → 環境差を疑う
- `which yohaku` の出力が `/Users/developer/.nodebrew/current/bin/yohaku` だった (symlink ヒント)
- `ls -la` で symlink を確認、`readlink` で参照先が `dist/cli.js` であることを確認
- `yohaku version` が silent exit 0 を返したことで isDirectInvoke 判定の失敗が確定
...
1. **CI で symlink 経由の動作確認を追加**: `npm link` または手動 symlink 作成 → `yohaku version` の出力確認を Phase 2 完了前に組み込む (improvement 候補)
