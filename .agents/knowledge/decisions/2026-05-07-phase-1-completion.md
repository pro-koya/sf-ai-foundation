...
type: decision
date: 2026-05-07
title: Phase 1 完了宣言 (core スケルトン + 中核実装)
status: active
supersedes:
superseded_by:
...
`packages/core/` の中核実装と関連基盤を完成させ、**Phase 1 の構造的完了** を宣言する。
ただし「実プロジェクトで `yohaku graph build` を走らせて検証」は次サイクル (Phase 2 の検証ゲート) で行う。
...
| 完了条件 | 状態 | 実装パス |
|---|---|---|
| `yohaku graph build` が `force-app/` を読み `.yohaku/graph.sqlite` を生成 | ✅ | `src/cli.ts` (cmdGraphBuild) + `src/graph/builder.ts` + `src/adapters/local/` + `src/graph/sqlite-store.ts` |
| `yohaku graph build --incremental` が差分だけ更新 | ✅ (基本実装) | sqlite-store の `writeAll(graph, "incremental")`。fine-grained 差分は Phase 7 で改善 |
| `yohaku graph schema` がスキーマ出力 | ✅ | `src/cli.ts` (cmdGraphSchema) |
| `yohaku render system-index` が描画 | ✅ | `src/render/render.ts` (renderSystemIndex) + `templates/system-index.eta` |
| `yohaku render objects` が描画 | ✅ | `src/render/render.ts` (renderObjects) + `templates/object.eta` |
...
| メタデータ機密性分類とマスキング規約 (テンプレート) | ✅ | `src/secrets/rules.ts` + `src/secrets/mask.ts` + `scaffold/.yohaku/secrets-rules.yaml.eta` |
...
- **secrets-rules.yaml の動的読み込み**: 現状は DEFAULT_RULES。`.yohaku/secrets-rules.yaml` の YAML パーサ統合は Phase 2 で対応
