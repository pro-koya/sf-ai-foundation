...
| # | 成果物 | 主要ファイル |
|---|---|---|
| 3-1 | `sfai diff` CLI (差分検出基盤) | `src/diff/{types,git,classify-files,diff,index}.ts`, テスト 11 件 |
...
| 3-6 | Code Analyzer SARIF 取り込み | `src/sarif/{types,parse,index}.ts`, `sfai diff --include-static-analysis`, テスト 3 件 |
...
1. Dev Edition 上の `sfai-trial` で 2〜3 ファイルの差分を作成 (例: 既存 Account に新 Custom Field 追加 + Apex 修正)
2. `git commit && sfai diff --from main --to HEAD --json --path-prefix force-app/`
