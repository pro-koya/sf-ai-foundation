...
**`yohaku sync` 1 コマンドで 49 ファイル (system-overview / executive 含む) が常に最新状態に再生成される** 状態を達成。
...
[yohaku] render complete: written=49 archived=0 warnings=0
```
...
| 自動懸念検出 | なし | yohaku-trial で MEDIUM 4 件を即時提示 |
...
| 既存 `.yohaku/graph.sqlite` に `body_json` カラムが無い | 古い DB を持つユーザでクラッシュ | `addColumnIfMissing` で起動時に自動 ALTER |
