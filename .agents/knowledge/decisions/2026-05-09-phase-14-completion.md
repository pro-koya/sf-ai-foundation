...
type: decision
date: 2026-05-09
title: Phase 14 完了 — AI_MANAGED ブロック自動生成基盤の拡張 (`/yohaku-explain` 複数 ID 対応)
status: completed
tags: [phase-14, completion, yohaku-explain, ai-managed, multi-id, registry]
---
...
| `/yohaku-explain` slash command | 未整備 (CLI のみ) | **scaffold に整備** (slash command + subagent) |
...
`packages/core/src/explain/index.ts`:
...
`packages/core/src/explain/block-registry.ts` を新規作成:
...
**`scaffold/.claude/commands/yohaku-explain.md.eta`**:
- 引数仕様: `<kind> <fqn> [block-id...]`
- 実行手順: graph query → explain-writer subagent → 一時 JSON → `yohaku explain-write`
...
| 検証項目 | 入力 | 結果 |
|---|---|---|
| Apex `AccountBalanceService` に 5 ID 一括書き込み (narrative / business-scenario / key-design-decisions / processing-overview-narrative / processing-details-narrative) | `/tmp/yohaku-explain-test/account-balance-service.json` | `updated=5 skipped=0` ✓ |
| Object `Order__c` に 3 ID 書き込み (summary / narrative / business-domain) — **新 kind 動作確認** | `/tmp/yohaku-explain-test/order-object.json` | `updated=3 skipped=0` ✓ |
| Flow `Order_AutoCreateShipmentOnApproval` に 4 ID 書き込み | `/tmp/yohaku-explain-test/flow.json` | `updated=4 skipped=0` ✓ |
...
| 不正 kind (`invalidKind`) を渡した場合 | invalid kind | `[yohaku] invalid --kind: invalidKind (allowed: apexClass \| apexTrigger \| flow \| ...)` (exit=2) ✓ |
| 書き込み後に `yohaku sync` を再実行 | — | warnings=0、written=69 維持 ✓ |
...
CLI (`yohaku explain-write`) は **書き戻し基盤** で、AI 推論は別の場所で実行される設計。Claude Code 環境では:
1. `/yohaku-explain` slash command が起動される
2. command が `explain-writer` subagent を呼び出す
3. subagent が知識グラフを query → DETERMINISTIC ファクトを集める → AI で文面生成 → JSON 出力
4. command が `yohaku explain-write` で書き戻し
...
- registry 検証 (異常系)
- 書き戻し後の `yohaku sync` 再実行で AI_MANAGED が保全されること (Phase 8 の保全契約継承)

実 AI 推論を含めた end-to-end は claude-code 環境で `/yohaku-explain` を実行することで実機検証可能。
...
- **実 AI 推論との end-to-end**: claude-code 環境で `/yohaku-explain` slash command を実機実行し、explain-writer subagent が DETERMINISTIC のみから自然文を生成するフローの検証
