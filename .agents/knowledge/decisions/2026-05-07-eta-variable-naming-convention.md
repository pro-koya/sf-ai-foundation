...
supersedes:
superseded_by:
tags: [scaffold, eta, naming-convention, yohaku-init]
---
...
`scaffold/` 配下のテンプレートが `yohaku init` で展開される際に参照する変数の **標準セットと命名規則** を以下に確定する。
...
| `it.yohakuVersion` | string | ✅ | (`package.json` から自動) | 利用者がインストールした yohaku のバージョン |
...
具体的な対応表は Phase 6 の `yohaku init` 実装時に scaffolding ロジックで確定する。
...
- このまま放置すると、Phase 6 で `yohaku init` を実装する際に変数名揺れ (例: `projectName` vs `name` vs `pname`) が発生し、テンプレート修正コストが膨らむ。
...
| D. 変数を JSON Schema で型定義し ajv で検証 | 採用 (派生対応) | 案 A と併用可能。Phase 6 で `yohaku init` の入力を JSON Schema で validate する |
...
- Phase 6 の `yohaku init` 実装時に、入力変数を JSON Schema で必須チェック
...
- IMPLEMENTATION_GUIDE.md の Phase 6 章に、`yohaku init` 実装時の変数仕様参照先として本 ADR をリンク
