...
**`sfai sync` 1 コマンドで 43 ファイルが常に最新状態に再生成される** 状態を達成。

## 達成内容

| # | 対象 | 出力先 | sfai-trial 実機件数 |
|---|---|---|---|
...
| 7-A4b | Profile | `docs/generated/permissions/profiles/<Name>.md` | 0 (sfai-trial には Profile 無し) |
...
`sfai sync` 再実行で **HUMAN_MANAGED ブロックを保護したまま再生成**。
...
| ドキュメント更新の手間 | `sfai sync` 1 コマンドで全種再生成。手作業ゼロ |
...
[sfai] render complete: written=43 archived=0 warnings=0
```
...
```bash
sfai render all                   # 全種一括 (Phase 7-A 新設)
sfai render flows                 # 個別
sfai render apex                  # 個別
sfai render triggers              # 個別
sfai render permissions           # PermissionSet + Profile 同時
sfai render validation-rules      # 個別
```

`sfai sync` も内部で `renderAll` を呼ぶように更新。`sfai init --bootstrap` も同様。
...
| sfai-trial で render される件数 | 43 ファイル (前回 11 → +32) |
