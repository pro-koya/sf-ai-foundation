...
- **A (LLM)**: `/yohaku-explain` slash command + `yohaku explain-write` CLI で、AI_MANAGED ブロックを自然言語で安全に上書きする経路を確立
...
| 8-B4 | yohaku-trial で実機検証 | 20 Apex クラス / 1 Flow / 2 Trigger 全件で Mermaid が壊れず、try/catch + nested if + for ループも正しく描ける |
| 8-A1 | `/yohaku-explain` slash command | `.claude/commands/yohaku-explain.md` を新設。AI_MANAGED 以外を変更しない厳守ルールを明記、`yohaku explain-write` を必ず経由する |
| 8-A2 | `yohaku explain-write` CLI | `src/explain/index.ts` の `applyExplain` をラップ。マーカー数の不変条件を実行時にチェック (改竄を拒否) |
| 8-A3 | yohaku-trial で実機検証 | `AccountBalanceService` と `OrderTrigger` で 2 つの AI_MANAGED ブロックを書き換え、後続 `yohaku sync` で保全されることを確認 |
...
これにより `/yohaku-explain` の出力が `yohaku sync` を超えて persistant になる。golden test (case-2) も新しい仕様に合わせて更新済。

## yohaku-trial での実機検証結果
...
### A 系 (`/yohaku-explain` 連携)

```bash
$ yohaku explain-write --kind apexClass --fqn AccountBalanceService \
    --project-root /Users/.../yohaku-trial --input blocks.json
[yohaku] explain-write: updated=2 skipped=0 → .../AccountBalanceService.md
```
...
その後 `yohaku sync` を再実行 → AI_MANAGED は **保全されたまま**、DETERMINISTIC (メソッド一覧 / Mermaid / 自動懸念) は最新化、HUMAN_MANAGED は無傷。
...
| `BlockKind` が小文字 (`ai_managed`) なのに `applyExplain` で大文字を渡していた (TS が catch しなかった) | yohaku explain-write の差し替えが空振り | `replaceBlockContent` への引数を小文字に修正 |
| AI_MANAGED が sync で常に上書きされていた (旧仕様) | `/yohaku-explain` の出力が消える | `mergeRender` で AI_MANAGED の customized 検出 → 保全に拡張 |
...
| Test Files | 27 | **28** |
| Tests | 127 | **169** (+42) |
| yohaku-trial で生成される Markdown | 49 | 49 (file 数は同じだが内容が大幅増) |
| 設計書セクション | メソッド名/SOQL/DML/Mermaid 概観 | + **メソッド単位制御フロー Mermaid** + **Flow 詳細フローチャート** |
| AI_MANAGED 経路 | 空のプレースホルダのみ | `/yohaku-explain` で内容が入り、sync 越えて永続 |
...
- 再現性 CI: `/yohaku-explain` を温度 0 / プロンプトハッシュ / N-run 一致でテストする仕組み
