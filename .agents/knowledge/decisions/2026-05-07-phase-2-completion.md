...
| `yohaku metrics` でトークン / コスト確認 | ✅ | record / show 別コマンド、period 別集計 |
...
1. **symlink isDirectInvoke バグ (CRITICAL)**: `npm link` 経由で `yohaku` が silent exit。`fs.realpathSync` で symlink resolve して比較する形に修正
...
- ✅ `yohaku init` `graph build` `render` `/onboard` `/explain` すべて期待通り動作
...
|---|---|
| サイクル数 | 8 (2-1〜2-8) |
| 新規 TypeScript モジュール (core) | 約 50 ファイル (合計約 4,500 行) |
...
- **AI コスト自動記録 (subagent → `yohaku metrics record`)**: 標準フックなし → Phase 3 / 5 で組み込み
...
1. **UX 改善サブサイクル (Phase 2.5)** — コマンド統合 (`yohaku init --bootstrap` / `yohaku sync` / `yohaku render` 引数省略) を実装
