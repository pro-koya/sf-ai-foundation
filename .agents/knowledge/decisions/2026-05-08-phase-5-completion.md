...
type: decision
date: 2026-05-08
title: Phase 5 (オンボーディング本格化) 完了 — sfai-trial で検証ゲート通過
status: active
...
Phase 5 のサイクル 5-1 〜 5-6 を完了。検証ゲートも sfai-trial で通過。**Phase 5 完了**。

## 達成したサイクル

| # | 成果物 | ファイル |
|---|---|---|
| 5-1 | context-map ローダ + 4 persona テンプレ | `src/onboarding/{types,context-map}.ts`, `scaffold/.sfai/context-map.yaml.eta` |
...
| 5-4 | `sfai onboard` CLI + `/onboard --role` 拡張 | `cli.ts` 3 サブコマンド、`scaffold/.claude/commands/onboard.md.eta` 全面更新 |
| 5-5 | FAQ 蓄積機構 + PII フィルタ | `src/onboarding/faq.ts` (parseDialogLog / extractFaq / renderFaqMarkdown、secrets/mask 再利用) |
| **5-6** | **検証ゲート** | **sfai-trial で 4 persona の context 取得 / state 記録 / FAQ 抽出 / review-assistant 起動を実機完走** |
...
| 指標 | 目標 | 実測 |
|---|---|---|
| `sfai onboard context --role new_joiner` 動作 | OK | ✅ 4 ドメイン + readOrder の `domains:*` 展開 |
| `sfai onboard state record-step` 動作 | OK | ✅ persona ごとに完走、entities 蓄積 |
| `sfai onboard state increment-questions` 動作 | OK | ✅ 累積カウント |
...
| 新規 scaffold setup | 1 件 (`.sfai/context-map.yaml.eta`) |
...
| sfai-trial 検証成果 | context-map / state / faq の 3 機構が実機動作 |
...
- もしくは Phase 5 の review-advisor / customer-impact-explainer を sfai-trial で実機検証してから次へ
