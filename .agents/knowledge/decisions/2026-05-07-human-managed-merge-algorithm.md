...
`sfai render` が再生成のたびに HUMAN_MANAGED ブロックの内容を **保全する** ためのマージアルゴリズムを以下の通り定める。本仕様はゴールデンテスト (`packages/sfai-core/tests/golden/render/`) の正本となる。
...
> **Phase 8 (2026-05-08) 更新**: AI_MANAGED は当初「再描画で上書き」だったが、`/sfai-explain` skill による書き換えを永続化するため、**既存内容がテンプレ既定値と異なれば保全** に仕様変更。`mergeRender` の `isCustomizedAiBlock` で判定する。
...
`sfai render --rename <oldName>=<newName>` で明示的に指定された場合のみ実行される。自動検出はしない (誤マージ防止)。
...
`sfai render` で対象エンティティが知識グラフから消失した場合、対応 Markdown を **削除しない**。
...
エラー時は人手介入を要求し、`sfai render --force` でも自動修復しない。

## ゴールデンテストケース (Phase 1 必須)

`packages/sfai-core/tests/golden/render/` に以下 6 ケースを配置する。各ケースは `input/` (旧ファイル + テンプレート出力) と `expected/` (期待結果) のペアを持つ。
...
- `packages/sfai-core/src/merge/` (Phase 1 で実装)
- `packages/sfai-core/src/types/render.ts` (型定義済み)
- `packages/sfai-core/tests/golden/render/case-{1..6}/` (Phase 1 で配置)
