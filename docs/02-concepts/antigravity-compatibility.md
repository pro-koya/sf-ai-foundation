...
yohakuforce の中核成果物は以下の **AI プラットフォーム非依存** な形式で書かれている:
...
| `yohaku` CLI | Node.js TypeScript | ✅ プラットフォーム非依存 |
...
- `.antigravity/tasks.json` 自動生成 (`yohaku init --antigravity`)
...
3. **`yohaku` CLI は無関係に動く**: Antigravity から Bash で `yohaku graph build` `yohaku diff` `yohaku onboard` を呼び出せる
4. **知識グラフ + Schema は完全互換**: Antigravity 上のエージェントが `yohaku graph query` を直接実行可能
