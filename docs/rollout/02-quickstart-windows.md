# SF-AI-Foundation — Windows 環境 Quickstart
...
git clone https://github.com/pro-koya/sf-ai-foundation.git
cd sf-ai-foundation
```
...
```powershell
node packages/sfai-core/dist/cli.js --version
# バージョン番号が表示されれば OK
```
...
```powershell
npm link --workspace=packages/sfai-core
sfai --version
# これで sfai コマンドが使えるようになります
```
...
### よくある問題 1: `sfai` コマンドが見つからない

**症状**: `sfai: command not found` または `sfai は認識されていません`
...
# それでも解消しない場合、フルパスで実行
node C:\Users\<username>\Documents\sf-ai-foundation\packages\sfai-core\dist\cli.js --version
```
...
### よくある問題 3: `sfai graph build` でパスエラーが出る
