...
```powershell
cd C:\Users\<username>\Documents\sf-ai-foundation\examples\sample-project
sfai --version
# 0.2.4 以上を確認
```
...
Scanning force-app/main/default/...
  ...
  ...
Graph built successfully -> .sfai/graph.sqlite
```
...
A: プロジェクトの `force-app/` がある Git リポジトリのルートで実行します。実際のプロジェクトに組み込む場合は `sfai init --bootstrap` で初期設定ファイルを一括生成できます。

**Q: `.sfai/graph.sqlite` はコミットするのか？**  
...
**Q: 日本語で出力されるのか？**  
A: テンプレートが日本語設定になっていれば日本語で出力されます。`sfai init --bootstrap` で初期化したプロジェクトにはデフォルトで日本語テンプレートが入ります。

**Q: `sfai render` を毎回手動で実行するのか？**  
A: `sfai sync` という 1 コマンドで、グラフ更新とドキュメント生成を一括実行できます。hooks を設定すれば `force-app/` を編集するたびに自動実行することも可能です。
...
A: Claude Code は Anthropic の有料サービスです。API 利用料がかかります。`sfai metrics show` で使用コストを確認できる仕組みが入っています。
...
`sfai diff` は Git の差分を使うため、事前に `git diff HEAD~1` で差分が出る状態を確認しておく。
...
**Q: `/classify-diff` と `sfai diff` の違いは？**  
A: `sfai diff` は CLI で差分を検出し分類結果を出力します。`/classify-diff` は Claude Code 内で対話しながら差分の詳細を掘り下げるときに使います。
...
1. `sfai graph build` — メタデータを知識グラフに変換（再現性の基盤）
2. `sfai render` — ドキュメントを自動生成（手動更新不要）
3. `/onboard` — 対話でプロジェクト概要を把握（新規参画者支援）
4. `sfai diff` — 差分を業務観点で分類（レビュー観点の整理）
