...
```
/yohaku-explain apexClass AccountBalanceService
/yohaku-explain apexTrigger OrderTrigger
/yohaku-explain flow Order_AutoCreateShipmentOnApproval
/yohaku-explain apexClass AccountBalanceService /Users/me/work/yohaku-trial
```
...
1. **AI_MANAGED ブロック以外は変更しない**
   - DETERMINISTIC ブロックは `yohaku sync` 専管。手で書き換えると次回の sync で必ず上書きされる
   - HUMAN_MANAGED ブロックは利用者の領域。AI が触ったらバグ
2. **マーカー (`<!-- AI_MANAGED_START id="..." -->` / `<!-- AI_MANAGED_END id="..." -->`) を破壊しない**
3. **書き戻しは必ず `yohaku explain-write` 経由**
...
各ブロックの本文を JSON ファイルに書き出して `yohaku explain-write` を呼ぶ。

> ⚠️ **`--input` のパス制約**: `yohaku explain-write` は **プロジェクトルート外のパスを拒否する** (例: `/tmp/...` は失敗する)。
> 一時 JSON は **プロジェクトルート内** に置くこと。推奨パスは `.yohaku/tmp-explain-<fqn>.json` (既存 `.gitignore` の `.yohaku/` に含まれるため追跡対象外)。
...
```bash
yohaku explain-write --kind <kind> --fqn <fqn> --project-root <root> --input <tmp.json>
```
...
`yohaku explain-write` は **指定された AI_MANAGED ブロックだけ** を `replaceBlockContent` 経由で差し替える。指定されていない id は触らない。
...
- 既存 Markdown が無い (まだ `yohaku sync` していない) → 「先に `yohaku sync` を実行してください」と案内
- `yohaku explain-write` が失敗 → エラー出力をそのまま提示し、勝手に再試行しない
