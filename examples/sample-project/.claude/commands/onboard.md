...
```bash
sfai onboard context --role <persona>
```
...
セッション開始時:
```bash
sfai onboard state record-step --role <persona> --step "session-start"
```

主要ステップ完了ごとに:
```bash
sfai onboard state record-step --role <persona> --step "<step-id>" --entities <comma-separated>
```
