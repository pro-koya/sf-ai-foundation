...
**まずルールベース** で `extractManualSteps` を呼ぶ (core 内部で決定的):

```bash
# 内部的には yohaku が呼ぶ。Claude Code からは change_summary を読み込んで結果を表示する想定。
```
...
> 自動生成 (yohaku release-prep): <createdAt>
...
```bash
yohaku metrics record --model claude-sonnet-4-6 --command /release-prep --in <est> --out <est>
```
