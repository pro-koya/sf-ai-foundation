...
- Layer 2 中間正本: `.yohaku/graph.sqlite` (正本から決定的に再生成可能)
...
| `@salesforce/cli` (sf CLI) | Salesforce 公式 (BSD-3) | peerDependency 宣言。`yohaku init` で存在チェック |

### 連携 (オプション / アダプタ経由 / 出力を入力に)

| ツール | 提供主体 | 連携方式 | 該当 Phase |
|---|---|---|---|
| Salesforce DX MCP Server (Beta) | Salesforce 公式 (Apache 2.0) | `adapters/dx-mcp/` で `yohaku init --source dx-mcp` を実装。ライブ org アクセスを担当 | Phase 2 で IF, Phase 6 で実装 |
| Salesforce Code Analyzer v5 | Salesforce 公式 (BSD-3) | SARIF 出力を `yohaku diff --include-static-analysis` で読み込み、差分分類器の入力に | Phase 3 |
...
- Agentforce DX / Vibes IDE — Phase 6 で Vibes IDE からの `yohaku` 呼び出し動作確認
...
- **月次トークン予算**: プロジェクトごとに `.yohaku/budget.yaml` で宣言。超過時はビルド失敗 (CI ブロック)
- **使用統計**: `yohaku metrics --period month` で集計、`docs/metrics/` に履歴を蓄積
...
| 項目 | 方針 | 該当 Phase |
|---|---|---|
| メタデータ機密性分類 | 顧客名 / メール / 内部 ID を検出する正規表現セットを `.yohaku/secrets-rules.yaml` に定義 | Phase 1 |
| マスキング | AI 送信前に分類済みフィールドを伏せ字化。マスキング前後をログに残し検証可能に | Phase 1 |
| ローカルストレージ暗号化 | `.yohaku/graph.sqlite` は OS キーチェーン連携 (option) で暗号化。デフォルトは平文 + gitignore | Phase 6 |
...
| v0.2.0 | 内部検証 (拡充) | ドキュメント完全化 (21 メタデータ種対応) + 処理フロー可視化 + AI 文面生成基盤 (`/yohaku-explain`) | ✅ リリース済 (2026-05-10) | 旧 Phase 7〜15 を統合 |
...
`yohaku` CLIを実装する。`@salesforce/source-deploy-retrieve` で `force-app/` を読み、SQLiteに構造化して格納する。格納されたグラフからMarkdownテンプレートを経由して文書を描画する。

### ゴール (完了条件)

- `yohaku graph build` が `force-app/` を読み `.yohaku/graph.sqlite` を生成する
- `yohaku graph build --incremental` が差分だけを更新する
- `yohaku graph schema` が現行スキーマ定義を出力する
- `yohaku render system-index` が `docs/generated/system-index.md` を描画する
- `yohaku render objects` が各オブジェクトのMarkdownを `docs/generated/objects/` に描画する
...
- **メタデータ機密性分類とマスキング規約** が `.yohaku/secrets-rules.yaml` (テンプレート) として整備されている
...
```
yohaku graph build [--incremental] [--source local|dx-mcp]   # 入力ソースは Phase 6 で dx-mcp 追加
yohaku graph query <sql>
yohaku graph schema [--format json|markdown]
yohaku render <target> [--output <path>]
yohaku validate <target>
yohaku metrics [--period day|week|month]                       # AI コスト計測 (Phase 2 から有効)
yohaku version
```
...
- `yohaku metrics` で AI 利用トークン数とコスト概算が出る
...
|---|---|---|
| graph-querier | SQLクエリで構造情報を取得 | Bash(yohaku graph query) |
| object-documenter | 単一オブジェクトの説明を生成 | Read, Bash(yohaku) |
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [{"type": "command", "command": "yohaku graph build --incremental --quiet"}]
    },
    {
      "matcher": "Edit|Write",
      "pathMatcher": "docs/generated/**",
      "hooks": [{"type": "command", "command": "yohaku validate --target $file"}]
    }
  ]
}
```
...
- **hooksは慎重に**: `yohaku graph build --incremental --quiet` の **wall-clock 2 秒以内** を目標。Node CLI cold-start で 600ms ほど消費するため 500ms 目標は非現実的だった (2026-05-07 計測)。大規模 org では hooks 無効化を許容
...
`yohaku diff` で決定的な差分を抽出し、subagentが意味分類する。並列実行でパフォーマンスを稼ぎ、変更サマリを自動生成する。

### ゴール (完了条件)

- `yohaku diff --from <ref> --to <ref> --json` が差分JSONを出力する
...
- `yohaku diff --include-static-analysis <sarif>` で **Code Analyzer v5 の SARIF を入力** に取れる ([ADR](./.agents/knowledge/decisions/2026-05-07-code-analyzer-sarif-integration.md))
...
- ロール別の読み順が `.yohaku/context-map.yaml` で定義される
...
- **進捗の記録**: どこまで理解したかを `.yohaku/onboarding-state.json` に記録 (ローカル、gitignore)
...
- npmパッケージ `@salesforce-ai/yohaku` として公開可能な状態
- Claude Codeプラグインとして `/plugin install` で導入可能
- Antigravity で同じ subagent/prompt が動作する (互換確認)
- **3 つの導入プロファイル** (minimal / standard / full) が `yohaku init --profile <name>` で選択可能
- **DX MCP アダプタ** (`adapters/dx-mcp/`) が動作し、`yohaku init --source dx-mcp` でローカル `force-app/` 不要で起動できる
- Agentforce Vibes IDE から `yohaku` CLI が呼び出せることを動作確認
...
各プロファイルは `yohaku init --profile <name>` で展開され、不要な subagent / command / hooks は無効化される。
...
```
@salesforce-ai/yohaku
├─ bin/yohaku                 # CLIエントリポイント
├─ dist/                    # コンパイル済みTS
├─ templates/               # 初期化テンプレート
└─ package.json
```
...
- `.antigravity/tasks.json` と `.antigravity/agent-workspaces.yaml` を生成する `yohaku init --antigravity` コマンド

**初期化コマンド**

```bash
yohaku init                   # Claude Code前提
yohaku init --antigravity     # Antigravity前提
yohaku init --both            # 両方
```
...
- 現参画 Salesforce プロジェクトに v0.2.0 を `yohaku init --bootstrap` で導入
- 週次運用ループ (`yohaku sync` → 差分確認 → AI 文面更新 → リリース準備) を 4 週継続
...
- **AI 推論基盤の機能拡張** (`/yohaku-explain` の機能追加、新 ExplainKind 追加 等)
...
├── .gitignore
│
├── packages/                        ← [ソース] OSS 本体 (monorepo, core 等)
│
...
- AIやhooksから `.yohaku/graph.sqlite` を直接INSERT/UPDATEしない
- 必ず `yohaku` CLIを経由する
...
- **判断**: 静的解析を本 OSS で再実装せず、Code Analyzer v5 の SARIF 出力を `yohaku diff --include-static-analysis` で取り込む
- **代替案**: 本 OSS で再実装 / 完全無視
